...
import { deleteAllPizzas, deletePizza, plusInstance, minusInstance } from '../redux/actions/cart'

export const Cart = () => {
    const dispatch = useDispatch();

    const { price, count, items } = useSelector((state) => state.cart);
    const pizzasInCart = Object.keys(items).map(key => { return items[key] });
    ...
    const onMinus = (id) => {
        dispatch(minusInstance(id));

    };
    const onPlus = (id) => {
        dispatch(plusInstance(id));

    };
    const onClickBuy = () => {
        alert('Спасибо за покупку, приходите снова! :)');
                ...
                {count > 0 ? (<div className="cart">
                    <div className="cart__top">
                        ...
                        <div className="cart__clear">
                            ...
                            <span onClick={onDeleteAll}>Очистить корзину</span>
                        </div>
                    </div>
                    <div className="content__items">

                        {pizzasInCart.map((pizzaWithPrice, index) => 
                        <CartItem key={index} 
                        id={pizzaWithPrice.items[0].id} 
                        name={pizzaWithPrice.items[0].name} 
                        type={pizzaWithPrice.items[0].type} 
                        size={pizzaWithPrice.items[0].size} 
                        price={pizzaWithPrice.price} 
                        count={pizzaWithPrice.items.length} 
                        onDeletePizza={onDeletePizzaById} 
                        onMinusInstance={onMinus}
                        onPlusInstance={onPlus}
                        />
                        )}
                    ...
                    <div className="cart__bottom">
                        ...
                        <div className="cart__bottom-buttons">
                            ...
                            <Button onClick={onClickBuy} className="pay-btn">
